...
public class BasketAudioManager : MonoBehaviour
{
    ...
    void Awake()
    {
        foreach (BasketSound sound in sounds)
        {
    ...
    {
        BasketSound s = Array.Find(sounds, sound => sound.name == name);
        if (s == null)
        {
            Debug.Log("No music");
            return;
    ...
    {
        BasketSound s = Array.Find(sounds, sound => sound.name == audio);
    ...
    {
        BasketSound s = Array.Find(sounds, sound => sound.name == audioClip);
        if (s != null && s.source.isPlaying)
        {
            s.source.Stop();
        }
